...
def _write_agent_workspace_instructions(
    ...
    (workspace_dir / "AGENTS.md").write_text(
        ...
        "\n"
        "## Research policy source\n"
        "\n"
        "- The current Experiment Index and active Research Problem Brief provide mounted problem-specific scientific policy, evidence standards, and Candidate-selection guidance. Repository maintainers must place Agent-required policy in those mounted sources rather than rely on outer-workspace instructions that are not part of the Agent Control Boundary.\n"
        "- The generated Workspace Configuration above owns enforceable execution ceilings and allowlists. Candidate source, manifests, and Agent handoffs cannot raise or replace them.\n"
        "- This generic Harness guidance does not override stricter current policy in mounted Research Problem or Experiment Index context. Do not infer additional approval or authorization gates from historical records; the outer operator command has already selected and bounded this Autonomy Step.\n"
        "- Treat completed-operation gates as Research History unless the current Experiment Index, active Research Problem context, or an active campaign pause says otherwise.\n"
        "\n"
