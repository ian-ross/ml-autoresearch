...
def _write_agent_workspace_instructions(
    ...
    (workspace_dir / "AGENTS.md").write_text(
        ...
        f"- The configured Candidate training wall-clock budget is {candidate_execution.training_wall_clock_timeout_seconds if candidate_execution.training_wall_clock_timeout_seconds is not None else 'not configured'}; when configured, the trusted Docker backend enforces it in seconds.\n"
