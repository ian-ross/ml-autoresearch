...
Prefer the Harness-recorded failure_classification when present. Approved classes are candidate_bug, contract_violation, resource_failure, harness_failure, bad_research_result, and unknown. A `non_finite_training_state` reason is a fail-fast numerical failure, normally `candidate_bug` for Candidate outputs/losses/gradients/parameters and `harness_failure` for a trusted adapter's non-finite aggregate metric. It is never a Resource Failure and must not trigger batch-size retry or duplicate training. A Repair Candidate is valid only for candidate bugs or contract issues that preserve hypothesis and Comparison Target.
