...
def test_readme_documents_pinned_docker_cuda_runtime() -> None:
    ...
    assert "workspace-specific runner image and also use Python 3.12" in readme
    assert "build-runtime-images" in readme
    assert "validate-runtime-images" in readme
