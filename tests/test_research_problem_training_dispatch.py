...
def _write_fake_problem_package(root: Path) -> None:
    ...
    (package / "research_problem.py").write_text(
        ...
        "        from pathlib import Path\n"
        "        return Path(data_config.get('dataset_root', '.')).resolve()\n"
        "\n"
        ...
        "        import torch\n"
        "        result = binary_segmentation_metrics(torch.sigmoid(logits) >= 0.5, target_mask >= 0.5)\n"
        "        return {'val/dice': result['dice']}\n"
        "\n"
        "    def selection_policy(self):\n"
        "        return 'val/dice', 'max'\n"
        "\n"
        "    def build_evaluation_dataset(self, *, data_config, resolved_manifest_path):\n"
        "        return SyntheticContrailDataset(2, seed=456, config=SyntheticContrailConfig(image_size=8))\n"
        "\n"
