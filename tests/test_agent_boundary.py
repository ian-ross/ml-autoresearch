...
''',
    ...
    assert "configured training ceiling is 100 epochs" in instructions
    assert "configured Candidate batch-size ceiling is 32" in instructions
    assert "configured scheduler allowlist is constant_lr, cosine_decay, reduce_on_plateau" in instructions
    assert "configured early-stopping policy is optional" in instructions
    assert "Candidate training wall-clock budget is 1800 seconds" in instructions
    ...
    assert "[candidate_execution]" in agent_candidate_config
    assert 'backend = "docker"' in agent_candidate_config
    assert "training_wall_clock_timeout_seconds = 1800" in agent_candidate_config
    assert "max_epochs = 100" in agent_candidate_config
    assert "max_batch_size = 32" in agent_candidate_config
    assert 'allowed_scheduler_policies = ["constant_lr", "cosine_decay", "reduce_on_plateau"]' in agent_candidate_config
    assert 'early_stopping_policy = "optional"' in agent_candidate_config
    assert "max_parallel_runs = 2" in agent_candidate_config
    assert "max_prediction_samples = 2" in agent_candidate_config
    assert 'prediction_sample_policy = "first_n"' in agent_candidate_config
    ...
    generated_policy = load_candidate_execution_config(tmp_path / "agent-work")
    assert generated_policy.training_wall_clock_timeout_seconds == 1800
    assert generated_policy.max_batch_size == 32
    assert generated_policy.allowed_scheduler_policies == (
        "constant_lr",
        "cosine_decay",
        "reduce_on_plateau",
    )
    assert generated_policy.early_stopping_policy == "optional"
