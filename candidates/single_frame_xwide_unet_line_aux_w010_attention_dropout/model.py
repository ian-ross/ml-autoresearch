"""Extra-wide U-Net with bottleneck dropout, line auxiliary logits, and skip attention gates."""
...
class AttentionGate(nn.Module):
    """Lightweight spatial gate for encoder skip features."""
...
class AttentionUp(nn.Module):
    """Bilinear upsample, attention-gate skip features, concatenate, then refine."""
    ...
    def __init__(self, in_channels: int, skip_channels: int, out_channels: int):
        ...
        self.attention = AttentionGate(
            ...
            intermediate_channels=max(skip_channels // 2, 16),
        )
...
class ExtraWideAttentionDropoutUNet(nn.Module):
    """Base-64 encoder/decoder with bottleneck dropout, skip gates, and mask/line heads."""
    ...
    def __init__(self, in_channels: int = 3, out_channels: int = 1, base_channels: int = 64):
        ...
        self.bottleneck_dropout = nn.Dropout2d(p=0.10)
        self.up3 = AttentionUp(base_channels * 8, base_channels * 4, base_channels * 4)
        self.up2 = AttentionUp(base_channels * 4, base_channels * 2, base_channels * 2)
        self.up1 = AttentionUp(base_channels * 2, base_channels, base_channels)
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_xwide_unet_line_aux_w010_attention_dropout requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_xwide_unet_line_aux_w010_attention_dropout produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_xwide_unet_line_aux_w010_attention_dropout requires line_logits auxiliary output")
    input_shape = input_spec.get("shape", [3, 128, 128])
    output_shape = output_spec.get("shape", [1, 128, 128])
    return ExtraWideAttentionDropoutUNet(
        in_channels=int(input_shape[0]),
        out_channels=int(output_shape[0]),
        base_channels=64,
    )
