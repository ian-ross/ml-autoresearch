# single_frame_xwide_unet_line_aux_w010_dropout_p005_epoch40

Extra-wide Single-Frame RGB U-Net Candidate Experiment for Ground-Camera Contrail Detection. This is a controlled dropout-strength probe of the current best 40-epoch p=0.075 bottleneck-dropout Line Target auxiliary model.

## Model Architecture

- Base-64 U-Net encoder/decoder with three skip-connected decoder stages.
- `Dropout2d(p=0.05)` after the bottleneck block.
...
The only intended scientific change from `single_frame_xwide_unet_line_aux_w010_dropout_p0075_epoch40` is reducing bottleneck dropout from p=0.075 to p=0.05 while keeping the 40-epoch budget. Any improvement must be checked for recall gain without material precision or empty-mask false-positive regression.
