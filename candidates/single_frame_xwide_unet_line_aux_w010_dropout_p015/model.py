"""Extra-wide U-Net-style Candidate Experiment with p=0.15 bottleneck dropout and line auxiliary logits."""
...
class ExtraWideDropoutP015UNet(nn.Module):
    """Base-64 encoder/decoder with p=0.15 bottleneck dropout and mask/line heads."""
    ...
    def __init__(self, in_channels: int = 3, out_channels: int = 1, base_channels: int = 64):
        ...
        self.bottleneck_dropout = nn.Dropout2d(p=0.15)
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_xwide_unet_line_aux_w010_dropout_p015 requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_xwide_unet_line_aux_w010_dropout_p015 produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_xwide_unet_line_aux_w010_dropout_p015 requires line_logits auxiliary output")
    input_shape = input_spec.get("shape", [3, 128, 128])
    output_shape = output_spec.get("shape", [1, 128, 128])
    return ExtraWideDropoutP015UNet(in_channels=int(input_shape[0]), out_channels=int(output_shape[0]), base_channels=64)
