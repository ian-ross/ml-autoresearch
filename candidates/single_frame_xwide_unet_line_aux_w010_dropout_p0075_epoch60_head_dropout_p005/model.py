"""Extra-wide U-Net with line auxiliary logits, bottleneck dropout, and light head-feature dropout."""
...
class ExtraWideHeadDropoutUNet(nn.Module):
    """Base-64 encoder/decoder with bottleneck and head-feature dropout plus mask/line heads."""
    ...
    def __init__(self, in_channels: int = 3, out_channels: int = 1, base_channels: int = 64):
        ...
        self.head_dropout = nn.Dropout2d(p=0.05)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        regularized_features = self.head_dropout(features)
        return {
            "mask_logits": self.mask_head(regularized_features),
            "line_logits": self.line_head(regularized_features),
        }


def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    candidate_name = "single_frame_xwide_unet_line_aux_w010_dropout_p0075_epoch60_head_dropout_p005"
    ...
    return ExtraWideHeadDropoutUNet(
