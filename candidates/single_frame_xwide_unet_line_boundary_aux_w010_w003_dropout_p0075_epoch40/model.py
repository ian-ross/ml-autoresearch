"""Extra-wide U-Net-style Candidate Experiment with line and conservative boundary auxiliary logits for a 40-epoch run."""
...
class ExtraWideBoundaryAuxUNet(nn.Module):
    """Base-64 encoder/decoder with mask, line, and boundary heads."""
    ...
    def __init__(self, in_channels: int = 3, out_channels: int = 1, base_channels: int = 64):
        ...
        self.boundary_head = nn.Conv2d(base_channels, out_channels, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            "line_logits": self.line_head(features),
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_xwide_unet_line_boundary_aux_w010_w003_dropout_p0075_epoch40 requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_xwide_unet_line_boundary_aux_w010_w003_dropout_p0075_epoch40 produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_xwide_unet_line_boundary_aux_w010_w003_dropout_p0075_epoch40 requires line_logits and boundary_logits auxiliary outputs")
    input_shape = input_spec.get("shape", [3, 128, 128])
    output_shape = output_spec.get("shape", [1, 128, 128])
    return ExtraWideBoundaryAuxUNet(in_channels=int(input_shape[0]), out_channels=int(output_shape[0]), base_channels=64)
