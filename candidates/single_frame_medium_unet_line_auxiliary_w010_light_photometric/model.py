"""Medium-capacity U-Net with mask/line logits and light photometric augmentation manifest."""
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_medium_unet_line_auxiliary_w010_light_photometric requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_medium_unet_line_auxiliary_w010_light_photometric produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_medium_unet_line_auxiliary_w010_light_photometric requires line_logits auxiliary output")
