"""Extra-wide U-Net-style Candidate Experiment with lighter bottleneck dropout, line auxiliary logits, and a longer 40-epoch budget."""
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_xwide_unet_line_aux_w010_dropout_p0075_epoch40 requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_xwide_unet_line_aux_w010_dropout_p0075_epoch40 produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_xwide_unet_line_aux_w010_dropout_p0075_epoch40 requires line_logits auxiliary output")
