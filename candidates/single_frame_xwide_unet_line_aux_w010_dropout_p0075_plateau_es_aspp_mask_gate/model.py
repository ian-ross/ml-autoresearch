"""Extra-wide U-Net with dilated bottleneck context and a mask-gated refinement head."""
...
class DilatedContextBlock(nn.Module):
    """Compact parallel dilated convolutions at bottleneck resolution."""

    def __init__(self, channels: int, branch_channels: int = 96):
        super().__init__()
        self.branches = nn.ModuleList(
            [
                ...
                nn.Sequential(
                    nn.Conv2d(channels, branch_channels, kernel_size=3, padding=2, dilation=2, bias=False),
                    nn.BatchNorm2d(branch_channels),
                    nn.ReLU(inplace=True),
                ),
                nn.Sequential(
                    nn.Conv2d(channels, branch_channels, kernel_size=3, padding=4, dilation=4, bias=False),
                    nn.BatchNorm2d(branch_channels),
                    nn.ReLU(inplace=True),
                ),
                nn.Sequential(
                    nn.Conv2d(channels, branch_channels, kernel_size=3, padding=6, dilation=6, bias=False),
                    nn.BatchNorm2d(branch_channels),
                    nn.ReLU(inplace=True),
                ),
            ]
...
class SpatialMaskGate(nn.Module):
    """Lightweight decoder-side spatial gate for mask logits.

    The gate is learned from final decoder features and only modulates the
    primary mask head.  The auxiliary line head remains attached to ungated
    features so the Harness-derived Line Target can continue supervising thin
    structure while the mask branch learns to suppress diffuse false-positive
    expansions.
    """
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        # Residual-style scaling keeps gradients and recall capacity available
        # while still allowing the mask branch to down-weight low-confidence
        # spatial regions.
        return features * (0.5 + self.gate(features))


class ExtraWideContextUNet(nn.Module):
    """Base-64 encoder/decoder with p=0.075 bottleneck dropout, context block, mask gate, and mask/line heads."""
    ...
    def __init__(self, in_channels: int = 3, out_channels: int = 1, base_channels: int = 64):
        ...
        self.context = DilatedContextBlock(base_channels * 8, branch_channels=96)
        ...
        self.mask_gate = SpatialMaskGate(base_channels)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        mask_features = self.mask_gate(features)
        return {"mask_logits": self.mask_head(mask_features), "line_logits": self.line_head(features)}


def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    candidate_name = "single_frame_xwide_unet_line_aux_w010_dropout_p0075_plateau_es_aspp_mask_gate"
    ...
    return ExtraWideContextUNet(in_channels=int(input_shape[0]), out_channels=int(output_shape[0]), base_channels=64)
