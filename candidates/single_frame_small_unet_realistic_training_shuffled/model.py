"""Small U-Net-style Candidate Experiment for single-frame contrail masks."""
...
class SmallUNet(nn.Module):
    """Compact encoder/decoder with skip connections and mask-logit output."""
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_small_unet_realistic_training_shuffled requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_small_unet_realistic_training_shuffled produces mask_logits output")
