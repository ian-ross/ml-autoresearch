# single_frame_small_unet_realistic_training_shuffled

A small standard U-Net-style Candidate Experiment for a more realistic Human-Guided Research Iteration on GVCCS, configured with the new candidate-selectable Sampling Policy.
...
- request Harness-owned `data.sampling_policy: deterministic_shuffle` so training example order is shuffled reproducibly while validation order remains stable.
