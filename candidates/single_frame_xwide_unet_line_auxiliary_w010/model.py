"""Extra-wide U-Net-style Candidate Experiment with mask and line auxiliary logits."""
...
class ExtraWideUNet(nn.Module):
    """Base-64 encoder/decoder with shared features and mask/line heads."""
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_xwide_unet_line_auxiliary_w010 requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_xwide_unet_line_auxiliary_w010 produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_xwide_unet_line_auxiliary_w010 requires line_logits auxiliary output")
    input_shape = input_spec.get("shape", [3, 128, 128])
    output_shape = output_spec.get("shape", [1, 128, 128])
    return ExtraWideUNet(in_channels=int(input_shape[0]), out_channels=int(output_shape[0]), base_channels=64)
