"""Extra-wide U-Net with line auxiliary logits and full-resolution residual refinement."""
...
class RefinementBlock(nn.Module):
    """Small full-resolution residual refinement block for thin mask details."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        refined = self.fuse(torch.cat([self.local_branch(x), self.context_branch(x)], dim=1))
        return x + refined


class ExtraWideRefinedUNet(nn.Module):
    """Base-64 encoder/decoder with light bottleneck dropout, refinement, and mask/line heads."""
    ...
    def __init__(self, in_channels: int = 3, out_channels: int = 1, base_channels: int = 64):
        ...
        self.refine = RefinementBlock(base_channels)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        x = self.up3(x, s3)
        x = self.up2(x, s2)
        features = self.refine(self.up1(x, s1))
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_xwide_unet_line_aux_w010_dropout_p0075_refine requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_xwide_unet_line_aux_w010_dropout_p0075_refine produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_xwide_unet_line_aux_w010_dropout_p0075_refine requires line_logits auxiliary output")
    input_shape = input_spec.get("shape", [3, 128, 128])
    output_shape = output_spec.get("shape", [1, 128, 128])
    return ExtraWideRefinedUNet(in_channels=int(input_shape[0]), out_channels=int(output_shape[0]), base_channels=64)
