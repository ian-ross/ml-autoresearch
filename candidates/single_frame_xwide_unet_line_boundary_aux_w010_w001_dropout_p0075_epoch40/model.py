"""Extra-wide U-Net with line and very-low-weight boundary auxiliary logits."""
...
class ExtraWideTinyBoundaryAuxUNet(nn.Module):
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_xwide_unet_line_boundary_aux_w010_w001_dropout_p0075_epoch40 requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_xwide_unet_line_boundary_aux_w010_w001_dropout_p0075_epoch40 produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_xwide_unet_line_boundary_aux_w010_w001_dropout_p0075_epoch40 requires line_logits and boundary_logits auxiliary outputs")
    input_shape = input_spec.get("shape", [3, 128, 128])
    output_shape = output_spec.get("shape", [1, 128, 128])
    return ExtraWideTinyBoundaryAuxUNet(in_channels=int(input_shape[0]), out_channels=int(output_shape[0]), base_channels=64)
