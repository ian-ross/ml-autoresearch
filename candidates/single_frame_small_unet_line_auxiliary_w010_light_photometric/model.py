"""Small U-Net-style Candidate Experiment with mask and line auxiliary logits."""
...
class DoubleConv(nn.Module):
    """Two 3x3 conv blocks used by the encoder, bottleneck, and decoder."""
...
class SmallUNet(nn.Module):
    """Compact encoder/decoder with shared features and mask/line logit heads."""
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_small_unet_line_auxiliary_w010_light_photometric requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_small_unet_line_auxiliary_w010_light_photometric produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_small_unet_line_auxiliary_w010_light_photometric requires line_logits auxiliary output")
    input_shape = input_spec.get("shape", [3, 128, 128])
    output_shape = output_spec.get("shape", [1, 128, 128])
    return SmallUNet(in_channels=int(input_shape[0]), out_channels=int(output_shape[0]), base_channels=16)
