"""Medium-capacity U-Net-style Candidate Experiment with lower-weight line auxiliary logits."""
...
def build_model(input_spec: dict, output_spec: dict) -> nn.Module:
    if input_spec["mode"] != "single_frame_rgb":
        raise ValueError("single_frame_medium_unet_line_auxiliary_w005 requires single_frame_rgb input")
    if output_spec["form"] != "mask_logits":
        raise ValueError("single_frame_medium_unet_line_auxiliary_w005 produces mask_logits output")
    ...
    if auxiliary_outputs != expected_auxiliary:
        raise ValueError("single_frame_medium_unet_line_auxiliary_w005 requires line_logits auxiliary output")
