...
The Agent Control Boundary must not mount the full Research Problem Repository. Instead, `prepare-agent-boundary` generates a curated read-only Agent Research Problem Snapshot containing importable provider Python sources, declared Research Problem Brief documents, Dataset Profile Artifacts, and their index, while excluding undeclared package resources and raw datasets. The Candidate Execution Boundary receives the trusted package root needed for full Harness-side imports and execution.
