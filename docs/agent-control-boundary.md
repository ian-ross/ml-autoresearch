...
events, updates canonical indexes when applicable, and writes a `.INGESTED.json` marker beside the source only after canonical state is updated. Candidate
...
Docker, GPU tools, cluster scheduler commands, direct training/evaluation scripts, Harness execution backends, or any command meant to submit or execute a
...
Harness-generated or trusted Research Problem package-generated path. It summarizes reproducible facts,
...
Dataset profile artifact examples include class balance, positive-pixel fraction, mask-area histogram, image/camera/source summaries, frame-sequence statistics, thin-structure summaries, known caveats, and Harness-selected qualitative examples. Qualitative examples are selected by Harness or Research
...
it should create a Capability Request for a Harness-generated dataset
profile artifact rather than probe raw training files directly.
...
implementation as the outer Harness. The image does not install PyTorch,
NVIDIA/CUDA libraries, Docker tooling, GPU utilities, and Run-execution
dependencies. Those heavy ML/runtime dependencies belong to the outer Harness and Candidate Execution Boundary, where Candidate Experiments are validated,
...
`prepare-agent-boundary`, `autonomy-step`, and `run-autonomous-iteration` fail before invoking the autonomy agent if pi-fort cannot be installed into the Agent
Workspace.
